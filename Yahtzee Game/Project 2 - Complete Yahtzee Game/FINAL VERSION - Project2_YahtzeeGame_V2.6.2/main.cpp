 ...
 * Created on February 13, 2022, 3:09 PM                                    *     
 * Purpose: Project 2: Yahtzee Game. Final version.                         *
 ***************************************************************************/
 ...
 *      1.12 Get Score Totals                                               *
 *      1.13 Display Final Scores                                           *
 ***************************************************************************/
...
void totScrs(bool[], int[]); //1.12 Get Score Totals
void dispFin(int[]); //1.13 Display Final Scores
...
int main(int argc, char** argv) {
    ...
    char cont; //Arbitrary input to pause end game mechanics
    ...
    while (ready=='y' || ready=='Y'){
        ...
        //Set all spaces to zero, open, and no string label
        ...
        while (!all){ //While all scorecard spaces NOT filled
            ...
            indx = getIndx(fullSp,dice); //Get array index from box input
            
            //Get category scores
            numSp[indx] = scoreFn(indx,dice, fullSp, valCnt, numSp);
            //Get total scores
            totScrs(fullSp, numSp);
            ...
            if (!all){
                ...
                cout<<"\n\t GAME OVER. ENTER ANY KEY TO CONTINUE: ";
                cin>>cont;
            }
 ...
 *                scoring processes.                                        *
 ***************************************************************************/
int getIndx(bool taken[], int dice[]){
    ...
    do{
        ...
        if (box=='1'){
            ...
            if (!taken[11]){
                ...
                if (taken[16]){ //If all four YB taken
                    inpOk = false; //SOL
                }
                else if (taken[15]){ //If three YB taken
                    indx = 16; //Set new index to YB4
                    inpOk = true;
                }
                else if (taken[14]){ //If two YB taken
                    indx = 15; //Set new index to YB3
                    inpOk = true;
                }
                else if (taken[13]){ //If one YB taken
                    indx = 14; //Set new index to YB2
                    inpOk = true;
                }
                else if (taken[11]){ //If ytz taken
                    indx = 13; //Set new index to YB1
                    inpOk = true;
                }
                else if (!taken[11]){
                    indx = 11; //Set to ytz
                ...
                cout<<"\t  ERROR: You cannot choose this box. Please choose"
                        " a different box: ";
            }
        }
        else if (box=='g'){
...
//1.12 GET SCORE TOTALS
void totScrs(bool full[], int numSp[]){
    
    //NOTE:
    //If category=0 AND is taken --> space will be blank
    
    //VALS[17]-->UPPER SECTION SUBTOTAL**************************************
    if (full[0]==full[1] && full[1]==full[2] && full[2]==full[3]
            && full[3]==full[4] && full[4]==full[5] && 
            full[5]==false){ //If 1-6 are all empty
        full[17] = false; //Mark upper section subtotal as open
        numSp[17] = 0; //Set total to zero
        //Combo of both open + zero makes space blank
    }
    else{ //If any of 1-6 is full
        full[17] = true; //Mark upper section subtotal as taken
        numSp[17] = numSp[0]+numSp[1]+numSp[2]+numSp[3]
            +numSp[4]+numSp[5]; //Upper section subtotal = sects 1+2+3+4+5+6
    }
    
    //VALS[18]-->BONUS*******************************************************
    if (full[0]==full[1] && full[1]==full[2] && full[2]==full[3]
            && full[3]==full[4] && full[4]==full[5] && 
            full[5]==true){ //If 1-6 are all taken
        full[18] = true; //Mark category as true --> display section
        if (numSp[17] >= 63){
            numSp[18] = 35; //Bonus +35 pts if upper section subtotal >= 63
        }
        else {
            numSp[18] = 0; //If under 63, no bonus
        }
    }
    else {
        full[18] = false; //Mark space as open
        numSp[18] = 0; //Set total bonus to zero
        //Combo of open + zero makes space blank
    }
    //VALS[19]-->UPPER SECTION TOTAL*****************************************
    if (full[17]){ //If upper section total full
        if (full[18]){ //If bonus section full
            full[19] = true; //Set total US to full
            numSp[19] = numSp[17] + numSp[18]; //Total US = US subtot + bonus
        }
        else { //If bonus section open
            full[19] = false; //Mark space as open
            numSp[19] = 0; //Set total bonus to zero
            //Combo of open + zero makes space blank
        }
    }
    else { //If upper section open
        full[18] = false; //Mark space as open
        numSp[18] = 0; //Set total bonus to zero
        //Combo of open + zero makes space blank
    }
    
    //VALS[20]-->YAHTZEE BONUS TOTAL*****************************************
    if (!full[13]){ //If first YB is open
        full[20] = false; //Mark space as open
        numSp[20] = 0; //Set YB to zero
        //Combo of open + zero makes space blank
    }
    else { //If the first YB is taken (aka: user has earned >= 1 YB
        full[20] = true;
        numSp[20] = numSp[13] + numSp[14] + numSp[15] + numSp[16];
        // Set YB total to sum of all four YB
    }
    
    //VALS[21]-->LOWER SECTION TOTAL*****************************************
    if (full[6]==full[7] && full[7]==full[8] && full[8]==full[9]
            && full[9]==full[10] && full[10]==full[11] && 
            full[11]==full[12] && full[12]==false){ //If all LS spots open
        full[21] = false; //Mark LS total as open
        numSp[21] = 0; //Set LS total to zero
        //Combo of open + zero makes space blank        
    }
    else {
        full[21] = true; //Mark space as taken
        numSp[21] = numSp[6] + numSp[7] + numSp[8] + numSp[9] + numSp[10]
                +numSp[11] + numSp[12] + numSp[20]; 
        //Set LS total to sum of all LS spots + YB total
    }
    
    //VALS[22]-->UPPER SECTION TOTAL (REMOVED DUE TO BUGS)*******************
    
    //VALS[23]-->GRAND TOTAL*************************************************
    if (full[6]==full[7] && full[7]==full[8] && full[8]==full[9]
            && full[9]==full[10] && full[10]==full[11] && 
            full[11]==full[12] && full[12]==true){ //If all LS spots full
        if (full[19]){ //If US total is filled
            full[23] = true; //Mark space as taken
            numSp[23] = numSp[21] + numSp[19]; //Set grand total to US + LS
        }
        else { //If some LS spots still open
            full[22] = false; //Mark section as open
            numSp[22] = 0; // Set section as equal to zero
            //Combo of open + zero makes space blank
        }
    }
    else { //If LS total is open
        full[22] = false; //Mark section as open
        numSp[22] = 0; // Set section as equal to zero
        //Combo of open + zero makes space blank
    }
...
void dispFin(int numSp[]){
    ...
    //Upper score total
    cout<<"\t\t\u2551 "<<right<<setw(25)<<"TOTAL OF UPPER SECTION:  "
            "\u250a"<<right<<setw(8)<<(numSp[17]+numSp[18])
            <<"     \u2551"<<endl;
    ...
    //Lower score total
    cout<<"\t\t\u2551 "<<right<<setw(25)<<"TOTAL OF LOWER "
            "SECTION:  \u250a"<<right<<setw(8)<<(numSp[21])
            <<"     \u2551"<<endl;
    ...
    //Grand total
    cout<<"\t\t\u2551 "<<right<<setw(28)<<"GRAND "
            "TOTAL:   \u250a"<<right<<setw(8)<<(numSp[23])
            <<"     \u2551"<<endl;
