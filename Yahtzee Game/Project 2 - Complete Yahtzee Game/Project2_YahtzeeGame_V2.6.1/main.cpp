 ...
 * Created on February 13, 2022, 12:38 AM                                   *     
 * Purpose: Project 2: Yahtzee Game. "Lower" section logic all coded.       *
 *                     Getting totals now. Progressively smoothing out any  *
 *                     bugs that might be found. Getting very close to      *
 *                     having a fully complete Yahtzee game.                *
 ***************************************************************************/
...
int main(int argc, char** argv) {
    ...
    while (ready=='y' || ready=='Y'){
        ...
        cout<<"\t  "<<right<<setw(25)<<"TOTAL OF UPPER SECTION: "
                <<numSp[22]<<endl;
        cout<<"\t  "<<right<<setw(25)<<"TOTAL OF LOWER SECTION: "
                <<numSp[21]<<endl;
        cout<<"\t  "<<right<<setw(25)<<"GRAND TOTAL: "<<numSp[23]<<endl;
        cout<<endl;
...
void totScrs(bool full[], int numSp[]){
    ...
    //VALS[22]-->UPPER SECTION TOTAL (AGAIN)*********************************
    if (full[0]==full[1] && full[1]==full[2] && full[2]==full[3]
            && full[3]==full[4] && full[4]==full[5] && 
            full[5]==true){ //If 1-6 are all taken
        full[22] = true; //Mark section as taken
        numSp[22] = numSp[19]; //Set section as equal to US total
    }
    else { //If all of US is not completed
        full[22] = false; //Mark section as open
        numSp[22] = 0; // Set section as equal to zero
        //Combo of open + zero makes space blank
    }
    //VALS[23]-->GRAND TOTAL*************************************************
    if (full[21]){ //If LS total is filled
        if (full[22]){ //If second US total is filled
            full[23] = true; //Mark space as taken
            numSp[23] = numSp[21] + numSp[22]; //Set grand total to US + LS
        }
        else { //If second US total is open
            full[22] = false; //Mark section as open
        numSp[22] = 0; // Set section as equal to zero
        //Combo of open + zero makes space blank
        }
