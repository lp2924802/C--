 ...
 * Created on February 11, 2022, 1:20 AM                                    *     
 * Purpose: Project 2: Yahtzee Game. Debugging string display in scorecard. *
 ***************************************************************************/
...
void getCard(fstream &card, string scLb[], bool taken[]){
    ...
    card<<"\u2551"<<left<<setw(16)<<"     Yahtzee"<<"\u2551"<<right
            <<setw(2)<<scLb[13]<<" \u250a"<<right<<setw(2)<<scLb[14]
            <<" \u250a"<<setw(2)<<scLb[15]<<" \u250a"
            <<setw(2)<<scLb[16]<<" \u2551\n";
...
int scoreFn(int &indx, int dice[], bool full[], int vals[]){
    ...
    number = indx + 1;
    bool foundA = false, foundB = false; //Flags for full house
    ...
    while (full[indx]){
        cout<<"\t ERROR: This space is already full. Please choose a "
                "different space: ";
        cin>>inp; //Get input
        indx=getIndx(inp,full); //Get new index value
    }
    
    cout<<"\t New index is: "<<indx<<endl;
    ...
    if (indx>=0 && indx<6){ //Get Upper Section scores
        ...
        switch (indx){
            ...
            case 8: { //Full House --> If x2 + x3, 25pts
                for (int i; i<VALS; i++){ //Loop to check each value
                    if (vals[i]==2) //If 2 of a kind
                        foundA=true;
                    else if (vals[i]==3) //If 3 of a kind
                        foundB=true;
                }
                if (foundA && foundB) //If both 2 and 3 of a kind together
                    score = 25;
                else
                    score = 0;
                break;
...
int getIndx(char box, bool taken[]){
    ...
    if (box=='1')
        ...
        indx = 5;
    else if (box=='a' || box=='A') //3 of a kind
        indx = 6;
    else if (box=='b' || box=='B') //4 of a kind
        indx = 7;
    else if (box=='c' || box=='C') //Full House
        indx = 8;
    else if (box=='d' || box=='D') //SM Straight
        indx = 9;
    else if (box=='e' || box=='E') //LG Straight
        indx = 10;
    else if (box=='f' || box=='F'){ //Yahtzee
