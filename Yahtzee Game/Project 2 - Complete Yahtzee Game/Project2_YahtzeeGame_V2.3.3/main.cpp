 ...
 * Created on February 9, 2022, 6:34 AM                                     *     
 * Purpose: Project 2: Yahtzee Game. Working on fixing bug where player     *
 *                     turns alternate back and forth between 1 and 2 in    *
 *                     an infinite loop, never ending the turn or           *
 *                     prompting the player to place their score in some    *
 *                     category. Will also be checking to see if there are  *
 *                     any variables or constants that aren't being used    *
 *                     and will remove any that might be found. Starting    *
 *                     to write scorecard arrays.                           *
 ***************************************************************************/
...
int scoreFn(char, int[], bool[]); //1.9 Score Dice
int scoreUS(char, int[]); //1.9.1 Score Upper Section
...
int main(int argc, char** argv) {
    ...
    //LOWER SECTION (LS) SCORECARD VARIABLES*********************************
    const int VALS2 = 11; //Number of spots in LS arrays
    int numLS[VALS2]; //Score in each box of LS
    bool fullLS[VALS2]; //Flag for if box is taken
    //***********************************************************************  
    */
    //OTHER SCORE/SCORECARD-RELATED VARIABLES AND CONSTANTS******************
    //const int VALS3 = 7; //Number of spots relating to total scores
    //int numTot[VALS3]; //Scores in totals boxes
    //bool fullTot[VALS3]; //Flag for if box is taken
    ...
    char ready; //New game? (Y/N) flag
    bool all = false; //Game over (all spots filled) flag
    ...
    while (ready=='y' || ready=='Y'){
        ...
        while (!all){ //While all scorecard spaces NOT filled
            ...
            score = scoreFn(box, dice, roll); //Get score
            //Function will be written later to validate this input
            
            cout<<score<<endl;
            cin>>box;
            
            //Give option to end game
            if (box=='x'){
                all = true;
            }
...
//1.9 SCORE DICE
int scoreFn(char box, int dice[], bool full[]){
...
//1.9.1 SCORE UPPER SECTION
/****************************************************************************
 *   Name:   scoreUS(char box, int dice[]) --> Score Upper Section          *
...
int scoreUS(char box, int dice[]){
    ...
    number = getNum(box,'v'); //Validate and convert box input to integer
