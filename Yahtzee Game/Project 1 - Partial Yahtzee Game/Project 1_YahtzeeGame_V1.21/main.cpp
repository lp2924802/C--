...
const char* chgDie(bool&,int,int,char); //Change die display status
const char* brVal(int); //Output bracketed dice value
...
void keepDie(bool &rDie1, bool &rDie2, bool &rDie3, bool &rDie4, bool &rDie5,
    ...
    const char chgKeep = 'x'; //Flag character to prompt keeping die
    const char chgRet = 'r'; //Flag character to prompt returning die
    char yesDie; //Number of the die to keep
    int yesInt; //Validated integer input
    string brNum1, brNum2, brNum3, brNum4, brNum5, brNum6; //Bracketed versions of numbers
    string strD1, strD2, strD3, strD4, strD5; //String names for each die
    ...
    //Establish default string value for dice
    //Dice #1
    if (&rDie1) //Die #1 will roll
        strD1 = to_string(die1); //Display number alone
    else //Die #1 will NOT roll
        strD1 = brVal(die1); //Get bracketed number
    //(All following strD# declaration statements follow same logic as Die #1)
    //Dice #2
    if (&rDie2) 
    ...
    while (keep=='y' || keep=='Y'){
        ...
        yesInt = getNum(yesDie); //Validate and convert yesDie input
        
        //Assign string to each dice depending on if dice is kept or not
        //NOTE: &rDie# = false --> dice will NOT roll --> dice is kept
        //      &rDie# = true --> dice will roll --> dice is NOT kept
        
        switch(yesInt){
            case 1: //Yes, pick up Die #1
                strD1 = chgDie(rDie1,die1,yesInt,chgKeep); //Switch die status to unrollable
                break;
            case 2: //Yes, pick up Die #2
                strD2 = chgDie(rDie2,die2,yesInt,chgKeep); //Switch die status to unrollable
                break;
            case 3: //Yes, pick up Die #3
                strD3 = chgDie(rDie3,die3,yesInt,chgKeep); //Switch die status to unrollable
                break;
            case 4: //Yes, pick up Die #4
                strD4 = chgDie(rDie4,die4,yesInt,chgKeep); //Switch die status to unrollable
                break;
            case 5: //Yes, pick up Die #5
                strD5 = chgDie(rDie5,die5,yesInt,chgKeep); //Switch die status to unrollable
                break;
        }
        cout<<"Current Dice: "<<strD1<<", "<<strD2<<", "<<strD3<<", "<<strD4<<", "<<strD5<<endl;
...
//CHANGE STATUS OF DICE --> (KEEP/RETURN)
//NOTE: Assume function only called when user wants to keep/return a die
const char* chgDie(bool &xDie, int xdieNum, int xdieVal, char keepRet){
    //Declare local variables
    string dieNew; //New string depiction of die
    const char* dieChars = &dieNew[0];
    char retDie; //(Y/N) on returning die
    char pickDie; //(Y/N) on keeping die
    
    //keepRet = 'k' --> User trying to keep die
    if(xDie && keepRet=='k'){ //Die still rollable
        xDie = false; //Switch to unrollable / kept
        dieNew = brVal(xdieVal); //Change to bracketed string
    }
    else if (!xDie && keepRet=='k'){ //Die not rollable
        cout<<"Die #"<<xdieNum<<" is already chosen. Would you like to return Die #"
                <<xdieNum<<"? (Y/N) ";
        cin>>retDie; //Get input
        yesNo(retDie); //Validate input
        if (retDie=='y' || retDie=='Y'){ //Yes, return
            xDie = true; //Switch to rollable / returned
            dieNew = to_string(xdieVal); //Change to only number
        }
        else if (retDie=='n' || retDie=='N'){ //No, don't return
            dieNew = brVal(xdieVal); //Return bracketed
        }
        else {
            cout<<"Bad code"<<endl;
        }
    }
    //keepRet = 'r' --> User trying to return die
    else if (!xDie && keepRet=='r'){ //Die not rollable
        xDie = true; //Switch to rollable / returned
        dieNew = to_string(xdieVal); //Change to only number
    }
    else if (xDie && keepRet=='r'){ //Die rollable
        cout<<"Die #"<<xdieNum<<" is not being held. Would you like to keep Die #"
                <<xdieNum<<"? (Y/N) ";
        cin>>pickDie; //Get input
        yesNo(pickDie); //Validate input
        if (pickDie=='y' || pickDie=='Y'){ //Yes, pick die
            xDie = false; //Switch to unrollable / kept
            dieNew = brVal(xdieVal); //Return bracketed
        }
        else if (pickDie=='n' || pickDie=='N'){ //No, don't pick die
            dieNew = to_string(xdieVal); //Return only number
        }
        else {
            cout<<"Bad code"<<endl;
        }
...
//RETURN BRACKETED DICE VALUE***************************************************
const char* brVal(int dieVal){
    //Declare local variables
    char *brDie; //Bracketed dice value
    
    switch(dieVal){
        case 1: //If dice value = 1
            brDie = "[1]";
            break;
        case 2: //If dice value = 2
            brDie = "[2]";
            break;
        case 3: //If dice value = 3
            brDie = "[3]";
            break;
        case 4: //If dice value = 4
            brDie = "[4]";
            break;
        case 5: //If dice value = 5
            brDie = "[5]";
            break;
        case 6: //If dice value = 6
            brDie = "[6]";
            break;
    }
    return brDie;
